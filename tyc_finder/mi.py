...
class other_scraper(object):
    # 定义天眼查爬虫
    @classmethod
    def tianyancha_scraper(cls, driver,
                           url_search,
                           table='all',
        ...
        """
        天眼查爬虫主程序。
        :param url_search: 公司名称，支持模糊或部分检索。比如"北京鸿智慧通实业有限公司"。
        ...
        table_dict = cls.scrapy(driver, table, use_default_exception, quit_driver=quit_driver,
                                change_page_interval=change_page_interval)
        # if export == 'xlsx':
        #     cls.gen_excel(table_dict, keyword)
        # elif export == 'json':
        #     cls.gen_json(table_dict, keyword)
        # else:
    ...
    # TODO:加入类别搜索功能
    @classmethod
    def get_announcement_info(cls, driver, table, change_page_interval=1):
        ...
        onclickflag = cls.tryonclick(announcement_table)
        if onclickflag == 1:
    ...
    @classmethod
    def change_page(cls, table, df, driver, change_page_interval=10):
        # TODO:抽象化：频繁变换点
        ...
        for _ in range(int(PageCount) - 1):
            ...
            ####################################################################################
            df2 = cls.get_table_info(table)  ## 应该可以更换不同的get_XXXX_info
            df = df.append(df2)
        return df

    @classmethod
    def change_tap(cls, driver, table, df):
        # TODO：完善change_tap函数。
    ...
    @classmethod
    def scrapy(cls, driver, table, use_default_exception,  quit_driver=True, change_page_interval=1):
        ...
        for x in range(len(tables) - 2):
            ...
            # 基本信息表：baseInfo，table有两个
                ...
                print('正在爬取' + 'baseInfo')
                try:
                    table_dict[name[x]] = cls.get_base_info(driver)
            ...
            # 公告的特殊处理：加入URL
                ...
                try:
                    table_dict[name[x]] = cls.get_announcement_info(driver, table,
                                                                    change_page_interval=change_page_interval)
                except Exception as e:
                    print(e)
            ...
            # TODO: 含头像的行未对齐
                ...
                df = cls.get_table_info(tables[x])
                onclickflag = cls.tryonclick(tables[x])
                ontapflag = cls.tryontap(tables[x])
                # 判断此表格是否有翻页功能
                if onclickflag == 1:
                    df = cls.change_page(tables[x], df, driver, change_page_interval=change_page_interval)
                #  if ontapflag == 1:
...
class Tianyancha(object):
    ...
    def tianyancha_scraper(driver,
        ...
        driver.get(url)
        table_dict = scrapy(driver, table, use_default_exception)
        # if export == 'xlsx':
